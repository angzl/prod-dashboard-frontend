...
/* Полный набор метрик для детального режима (Детализация по проектам):
   активные, ТО сегодня, ТО вчера, ТО 3 дня, разрыв, БС. */
const METRICS_DETAIL = [
  ...
  { key: 'gap_pct', label: 'Разрыв', isPct: true, invert: true },
  { key: 'bs_total',  label: 'БС всего',  noBg: true },
  {
    key:   'bs_online',
    label: 'БС онлайн',
...
function AllProjectsHistoryTable({ partners, days = 30, mode = 'daily' }) {
  ...
  // В timeline-режиме (Сводка / Весь прод) показываем сокращённый набор
  // метрик, в детальном — полный (Т0 сегодня, вчера, разрыв и т.д.).
  const METRICS = isTimeline ? METRICS_TIMELINE : METRICS_DETAIL;
